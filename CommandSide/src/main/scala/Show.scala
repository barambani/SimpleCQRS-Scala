...
object Show {
  ...
  implicit class ShowSyntax[A](a: A) {
    def show(implicit SH: Show[A]): String = SH stringFor a
  }
}
...
trait ErrorsShow {
  ...
  implicit object ImpossibleToCreate extends Show[ImpossibleToCreate] {
  ...
  implicit object ErrorMessageShow extends Show[ErrorMessage] {
    def stringFor(x: ErrorMessage): String = x match {
      case e: ImpossibleToCreate        => Show[ImpossibleToCreate].stringFor(e)
